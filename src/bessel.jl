...
function discrete_bessel{N,T<:Real}(x::Frequency{N,T})
    @assert in(slice(x), [0,N-1])
    x = Complex128[ exp(im * (x.λ * cos(rotate(x.ω, k)))) for k in 0:N-1 ]
    return circshift(fft(x), -1)
end
...
function discrete_bessel_matrix{N,T<:Real}(E::BispectralSet{N,T}, F::BispectralSet{N,T}; verbose = true)
    verbose && (length(E) != length(F)) ? println("Non-square Jn matrices") : nothing

    J = Array(Complex128, N, size(E,1), size(F,1))

    # we store the values in a dict to avoid computing them more than once
    coeff = Dict{Frequency{N,T},Vector{Complex128}}()
